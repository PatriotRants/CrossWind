...
static RunStatus Configure(ref AgentState agentState)
{
    ...
    if (PrototypeInfo.Any())
    {
        TestContext.Current.AddPrototypes(PrototypeInfo);
    }
...
public static partial class Program
{
    ...
    static readonly string[] PROTO = { "-p", "--proto" };
    ...
    static Dictionary<Type, ConstructorInfo> PrototypeInfo { get; } = new();
    ...
    */
    ...
    static void LoadTestCases(Type[] types)
    {
    ...
    static void LoadPrototypes(Type[] types)
    {
        var paramTypes = new Type[] { };

        foreach (var t in types.Where(pt => pt.GetCustomAttributes().OfType<PrototypeAttribute>().Any()))
        {
            var ctor = t.GetConstructor(BindingFlags.Instance | BindingFlags.Public, paramTypes);

            if (ctor != null)
            { PrototypeInfo.Add(t, ctor); }
        }
    }
    /// <summary>
    /// Configure Logging In/Out channels
    /// </summary>
    private static void ConfigureLogger(Stream outputLogStream)
    {
